...
(function () {
    ...
    function recruitingHtml() {
        ...
        state.employees.forEach(function (employee) {
            ...
            positionCounts[name] = (positionCounts[name] || 0) + 1;
        });
        ...
        html += '<div class="tccc-grid2">';
        html += '<section class="tccc-panel"><div class="tccc-panel-head"><h3>Current staffing mix</h3><span>Based on active employee positions</span></div><div class="tccc-position-mix">';
        Object.keys(positionCounts).sort().forEach(function (position) {
            html += '<div><span>' + esc(position) + '</span><strong>' + esc(positionCounts[position]) + '</strong></div>';
        });
        ...
        html += '</section>';
        html += '<div class="tccc-note"><strong>Recruiting data:</strong> Torn provides each current application\'s player level, manual labor, intelligence, endurance, application message, status, and expiration. This page intentionally shows the raw recruiting facts rather than assigning applicants an arbitrary “good/bad” score. The next layer can add role-specific targets so you can compare an applicant against the actual position you need to fill.</div>';
        return html;
    }
